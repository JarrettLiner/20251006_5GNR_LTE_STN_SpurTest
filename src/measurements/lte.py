...
# Description: This module provides the driver for LTE measurements using VSA and VSG instruments.
#              It handles configuration, EVM, and ACLR measurements for LTE signals.
...
class std_insr_driver:
    ...
    def __init__(self, freq=6e9, pwr=-10.0, waveform_file=None, setup_file=None):
        ...
        """
        logger.info(f"Initializing LTE driver with freq={freq / 1e9:.3f}GHz, pwr={pwr}dBm, "
        ...
        self.rb = self.waveform_params.get("resource_blocks", 100)
        self.rbo = self.waveform_params.get("resource_block_offset", 0)
        self.bw = self.waveform_params.get("channel_bandwidth_mhz", 20)
        self.mod = self.waveform_params.get("modulation_type", "QAM256")
        self.dupl = self.waveform_params.get("duplexing", "FDD")
        self.ldir = self.waveform_params.get("link_direction", "UL")
    ...
    def _validate_file_names(self, waveform_file, setup_file):
        ...
        waveform_pattern = r'^LTE_(UL|DL)_(\d+MHz)_(QPSK|16QAM|64QAM|256QAM|1024QAM)_(\d+RB)_(\d+RBO)\.wv$'
        setup_pattern = r'^LTE_(UL|DL)_(\d+MHz)_(QPSK|16QAM|64QAM|256QAM|1024QAM)_(\d+RB)_(\d+RBO)\.dfl$'
        for file_path, file_type, pattern in [(waveform_file, "waveform", waveform_pattern), (setup_file, "setup", setup_pattern)]:
            if file_path:
    ...
    def _extract_waveform_params(self, waveform_file):
        ...
        pattern = r'^LTE_(UL|DL)_(\d+MHz)_(QPSK|16QAM|64QAM|256QAM|1024QAM)_15kHz_(\d+RB)_(\d+RBO)\.wv$'
        ...
        try:
            ...
            return {
                ...
                "channel_bandwidth_mhz": int(match.group(2).replace("MHz", "")),
                "modulation_type": match.group(3),
                "subcarrier_spacing_khz": 15,  # Fixed for LTE
                "resource_blocks": int(match.group(4).replace("RB", "")),
                "resource_block_offset": int(match.group(5).replace("RBO", "")),
    ...
    @method_timer
    def VSG_Config(self):
        """Configure VSG for LTE signal generation by recalling waveform."""
        try:
    ...
    @method_timer
    def VSA_Config(self, freq=None):
        """Configure VSA for LTE measurement by recalling setup file."""
        try:
    ...
    def VSA_get_info(self):
        ...
        if self.waveform_params:
            ...
            config = (f"{self.freq / 1e9:.3f}GHz_"
                      f"{params['channel_bandwidth_mhz']}MHz_"
            ...
            config = (f"{self.freq / 1e9:.3f}GHz_{self.bw}MHz_{self.dupl}_{self.ldir}_"
                      f"15kHz_{self.rb}RB_{self.rbo}RBO_{self.mod}")
            if self.waveform_file:
                config += f"_waveform_{os.path.basename(self.waveform_file)}"
            if self.setup_file:
                config += f"_setup_{os.path.basename(self.setup_file)}"
    ...
    def VSA_get_EVM(self):
        ...
        try:
            self.VSA.write(':CONF:LTE:MEAS EVM;*OPC')
            self.VSA.query('INIT:IMM;*OPC?')
